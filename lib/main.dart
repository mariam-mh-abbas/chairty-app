...
void main() async {
  ...
  if (!Platform.isWindows) {
    ...
    NotificationService().listenTokenRefresh((newToken) async {
      await DeviceService().registerDevice(fcmToken: newToken);
    });
  } else {
    print("🔹 Running on Windows: skipping Firebase and notifications");
  }
  ...
  Widget build(BuildContext context) {
    return MultiBlocProvider(
        providers: [
          ...
          BlocProvider(
            create: (context) =>
                RequestBloc(RequestService())..add(HelpRequestEvent()),
          ),
          BlocProvider(
              create: (context) =>
        ...
        child: ScreenUtilInit(
            ...
            builder: (context, child) {
              return MaterialApp(
                  theme: ThemeData(
                    fontFamily:
                        context.locale.languageCode == 'en' ? 'ro' : 'tj',
                  ),
