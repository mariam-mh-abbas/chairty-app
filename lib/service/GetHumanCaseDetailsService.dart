...
class Gethumancasedetailsservice extends Baseservice  {
 Future<Map<String,dynamic>?> gethumancasedetailsservice ({required int id}) async{
try {
  final savedLang = await SharedPrefs.getLanguage() ?? 'en';
  responce = await dio.get("$baseURL/${ApiResourses.humancaseDetails(id)}",options: Options(
    headers: {
